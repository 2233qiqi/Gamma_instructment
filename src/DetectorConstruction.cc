...
G4VPhysicalVolume *DetectorConstruction::Construct()
{
   ...
   //内部环境
    ...
    //可视化
    G4VisAttributes* visGe = new G4VisAttributes(G4Color(1.0, 0.0, 0.0, 0.8));
    visGe->SetForceSolid(true);
    fLogicGeCrystal->SetVisAttributes(visGe);

    G4VisAttributes* visAl = new G4VisAttributes(G4Color(0.8, 0.8, 0.8, 0.3));
    visAl->SetForceWireframe(true);
    logicCapTube->SetVisAttributes(visAl);
    logicCapWin->SetVisAttributes(visAl);
